...
4. Um bloco por critério avaliado (todos os que constarem no resumo, na ordem em que aparecem): um cabeçalho curto só com o nome do critério (ex.: "Critério A"), seguido de um parágrafo corrido no padrão "atribuí N pontos, em um máximo de M. [justificativa em linguagem natural, baseada na justificativa técnica fornecida, mas reescrita em tom fluido]". Nunca liste subitens ou marcadores dentro do parágrafo do critério. NUNCA inverta, contradiga ou distorça o sentido factual da justificativa técnica ao reescrevê-la -- mude só o estilo, nunca o que ela afirma. Em particular: nem todo critério com nota abaixo do máximo é por "documentação insuficiente" -- às vezes é porque uma condição desqualificante foi CONFIRMADA (ex.: autodeclaração de participação anterior no Ciclo 1, que zera o critério correspondente por definição do edital, não por falta de prova). Nesses casos, explique que a condição foi confirmada, nunca reescreva como se fosse ausência de comprovação.
